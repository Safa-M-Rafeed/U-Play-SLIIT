...
export default function TournamentDetail() {
  ...
  // Where "back" goes depends on role
  const listPath      = `/${role}/tournaments`;
  const dashboardPath = { admin: '/admin', captain: '/captain', student: '/student' }[role] ?? '/';
  // Captain register route — ready for when teammate's page is merged
  const registerPath  = `/captain/tournaments/${id}/register`;
  ...
  // Is registration open? (used to enable/disable captain's register button)
    ...
    <div className='glass-page'>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
              ...
              <span style={{
                ...
                background: statusColor(tournament) + '22', color: statusColor(tournament),
                border: `1px solid ${statusColor(tournament)}44`,
              }}>
                {tournament.status}
              </span>
            </div>
          ...
          {/* Action buttons — differ per role */}
          <div style={{ display: 'flex', gap: '8px' }}>

            {/* Admin: Edit / Clone / Delete */}
            {role === 'admin' && (
              <>
                <button className='glass-btn-ghost' style={{ color: '#10b981', borderColor: 'rgba(16,185,129,0.4)' }}
                  onClick={() => navigate(`/admin/tournaments/${id}/edit`)}>
                  Edit
                </button>
                <button className='glass-btn-ghost' onClick={handleClone}>Clone</button>
                <button className='glass-btn-ghost' style={{ color: '#f87171', borderColor: 'rgba(239,68,68,0.4)' }}
            ...
            {/* Captain: Register Team button */}
            {role === 'captain' && (
              <button
                className='glass-btn-primary'
                disabled={!registrationOpen}
                onClick={() => navigate(registerPath)}
                title={!registrationOpen ? 'Registration is closed for this tournament' : ''}
                style={!registrationOpen ? { opacity: 0.5, cursor: 'not-allowed' } : {}}>
                {registrationOpen ? 'Register Team' : 'Registration Closed'}
              </button>
            )}

            {/* Student: no action buttons */}
          </div>
        </div>

        {/* Captain: registration status banner */}
        {role === 'captain' && (
          <div style={{
            marginBottom: '24px', padding: '14px 20px', borderRadius: '12px',
            background: registrationOpen ? 'rgba(16,185,129,0.08)' : 'rgba(107,114,128,0.08)',
            border: `1px solid ${registrationOpen ? 'rgba(16,185,129,0.25)' : 'rgba(107,114,128,0.25)'}`,
            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
          }}>
            <div>
              <p style={{ color: registrationOpen ? '#10b981' : '#9ca3af', fontWeight: '600', margin: '0 0 2px', fontSize: '14px' }}>
                {registrationOpen ? 'Registration is open' : 'Registration is closed'}
              </p>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', margin: 0 }}>
                Deadline: {new Date(tournament.registrationDeadline).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                &nbsp;·&nbsp;
                {tournament.maxTeams - tournament.registeredTeams} slot{tournament.maxTeams - tournament.registeredTeams !== 1 ? 's' : ''} remaining
              </p>
            </div>
            {registrationOpen && (
              <button className='glass-btn-primary' onClick={() => navigate(registerPath)}>
                Register Team →
              </button>
            )}
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '24px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {/* Info grid — visible to all roles */}
            ...
            {role === 'admin' && tournament.conflicts?.length > 0 && (
              <div className='glass-card'>
            ...
            <div className='glass-card'>
              ...
              {role === 'admin' && (
                <form onSubmit={handleAnn} style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px' }}>
                  <div>
                    <input
                      value={ann.title}
                      onChange={e => { setAnn({ ...ann, title: e.target.value }); setAnnErrors({ ...annErrors, title: null }); }}
                      placeholder='Announcement title'
                      className={ic(annErrors.title)}
                    />
                  ...
                  <div>
                    <textarea
                      value={ann.message}
                      onChange={e => { setAnn({ ...ann, message: e.target.value }); setAnnErrors({ ...annErrors, message: null }); }}
                      placeholder='Write your announcement...'
                      rows={3}
                      className={ic(annErrors.message)}
              ...
              {/* Announcement list — visible to all roles */}
              {tournament.announcements?.length === 0 && (
                <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '14px' }}>No announcements yet.</p>
              )}
