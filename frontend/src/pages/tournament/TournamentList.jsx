...
export default function TournamentList() {
  ...
  // Each role stays within its own route prefix
  const dashboardPath = { admin: '/admin', captain: '/captain', student: '/student' }[role] ?? '/';
  const detailPath    = (id) => `/${role}/tournaments/${id}`;
  ...
  return (
    <div className='glass-page'>
      ...
      <nav className='glass-nav'>
        ...
        {/* Right side of nav differs by role */}
        {role === 'admin' && (
      ...
      <div style={{ padding: '24px' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ fontSize: '36px', fontWeight: '800', color: '#fff', margin: '0 0 8px', lineHeight: 1 }}>
            {role === 'admin'
              ...
              : role === 'captain'
              ? <>Available <span style={{ color: '#10b981' }}>tournaments</ span></>
              : <>Browse <span style={{ color: '#10b981' }}>tournaments</span></>
            }
          </h1>
        ...
        {/* ── Main layout: admin gets sidebar, others get full-width list ── */}
        <div style={{
          display: 'grid',
          gridTemplateColumns: role === 'admin' ? '1fr 320px' : '1fr',
          gap: '24px',
        }}>
          ...
          <div>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
              ...
              <input
                placeholder='Search tournaments...' value={search}
            ...
            {error && (
              <div style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '16px', padding: '20px', textAlign: 'center' }}>
                ...
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', margin: 0 }}>Check VITE_API_URL in .env and ensure backend is running on port 8000</p>
              </div>
            )}
            ...
            {!loading && !error && filtered.length === 0 && (
              <div style={{ textAlign: 'center', padding: '60px 20px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px' }}>
                ...
                {role === 'admin'
                  ...
                  : <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', margin: 0 }}>Check back soon for upcoming tournaments</p>
                }
              </div>
            )}
